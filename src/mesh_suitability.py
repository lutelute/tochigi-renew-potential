...
# 栃木県範囲 (行政区域の実範囲に余裕を持たせる)
XMIN, YMIN, XMAX, YMAX = 139.32, 36.19, 140.30, 37.16
...
def create_mesh(resolution_m: int) -> gpd.GeoDataFrame:
    """メッシュグリッドを生成"""
    # 度単位への変換 (概算)
    res_deg = resolution_m / 111000.0
    res_deg_x = resolution_m / (111000.0 * np.cos(np.radians(36.7)))
...
def clip_to_tochigi(mesh: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """栃木県行政区域でクリップ"""
    ...
    tochigi_boundary = admin.union_all()
    mask = mesh.geometry.intersects(tochigi_boundary)
    clipped = mesh[mask].copy().reset_index(drop=True)
    print(f"  栃木県クリップ: {len(clipped)} セル")
    return clipped


def score_slope(mesh: gpd.GeoDataFrame) -> np.ndarray:
...
def score_land_use(mesh: gpd.GeoDataFrame, mode: str = "ground") -> np.ndarray:
...
def score_elevation(mesh: gpd.GeoDataFrame) -> np.ndarray:
...
def compute_mesh(resolution_m: int, lines, subs, mode: str = "ground"):
    """指定解像度でメッシュ計算し mesh_gdf を返す
    mode: "ground" (大規模地上設置) or "rooftop" (屋根置き)
    """
    print(f"\n--- {resolution_m}m メッシュ ({mode}) ---")
    mesh = create_mesh(resolution_m)
    mesh = clip_to_tochigi(mesh)

    mesh["score_slope"] = score_slope(mesh)
    mesh["score_grid_dist"] = score_grid_distance(mesh, lines)
    mesh["score_sub_dist"] = score_substation_distance(mesh, subs)
    mesh["score_land_use"] = score_land_use(mesh, mode=mode)
    mesh["score_elevation"] = score_elevation(mesh)
    ...
    # GeoJSON保存
    out = OUTPUT_DIR / f"tochigi_mesh_{resolution_m}m.geojson"
...
def main():
    ...
    for res in resolutions:
        meshes_ground[res] = compute_mesh(res, lines, subs, mode="ground")
        meshes_rooftop[res] = compute_mesh(res, lines, subs, mode="rooftop")

    # 統合マップ生成 (レイヤー切替可能)
    print("\n[3] 統合マップ生成")
    import folium

    m = folium.Map(location=[36.65, 139.88], zoom_start=9, tiles=None)
    ...
    # 道路レイヤー (軽量: 主要道路のみ)
    road_geojson = GRID_DIR / "tochigi_roads.geojson"
    if road_geojson.exists():
        ...
        # OSM Overpass APIから主要道路を取得
        print("  道路データ取得中 (Overpass API)...")
        try:
            import requests
            query = """
            [out:json][timeout:60];
            ...
            if resp.status_code == 200:
                ...
                if road_features:
                    ...
                    roads_gdf.to_file(road_geojson, driver="GeoJSON")
                    # simplify for display
                    roads_gdf["geometry"] = roads_gdf["geometry"].simplify(0.0005)
                    fg_roads = folium.FeatureGroup(name="主要道路", show=False)
                    folium.GeoJson(
                        roads_gdf.to_json(),
                        style_function=lambda x: {
                    ...
                    fg_roads.add_to(m)
                    print(f"  道路レイヤー追加: {len(roads_gdf)} 本 (OSM)")
                else:
                    print("  WARNING: 道路データ取得なし")
            else:
                print(f"  WARNING: Overpass API error {resp.status_code}")
        except Exception as e:
            print(f"  WARNING: 道路取得エラー: {e}")
    ...
    for _, row in subs[subs["voltage_kv"] >= 66].iterrows():
        ...
        v = row["voltage_kv"]
        centroid = row.geometry.centroid
        sub_entries.append((name, v, centroid.y, centroid.x))
    ...
    for _, row in lines[lines["voltage_kv"] >= 66].iterrows():
        ...
        line_entries.append((name, v, centroid.y, centroid.x))

    line_entries.sort(key=lambda x: x[0])

    # HTML生成
    sub_items = ""
    for name, v, lat, lon in sub_entries:
        color = ("#ff0000" if v >= 500 else "#ff6600" if v >= 275
                 else "#0066ff" if v >= 154 else "#00aa44")
        sub_items += (f'<div class="list-item" onclick="map.setView([{lat},{lon}],14)" '
                      f'style="cursor:pointer;padding:2px 4px;border-left:3px solid {color}">'
                      f'<small style="color:{color}">{v:.0f}kV</small> {name}</div>')

    line_items = ""
    for name, v, lat, lon in line_entries[:100]:  # 上位100件
        color = ("#ff0000" if v >= 500 else "#ff6600" if v >= 275
                 else "#0066ff" if v >= 154 else "#00aa44")
        line_items += (f'<div class="list-item" onclick="map.setView([{lat},{lon}],13)" '
                       f'style="cursor:pointer;padding:2px 4px;border-left:3px solid {color}">'
                       f'<small style="color:{color}">{v:.0f}kV</small> {name}</div>')

    list_panel_html = f"""
    <div id="listPanel" style="position:fixed;bottom:30px;right:10px;z-index:1000;
                background:white;padding:8px;border:2px solid grey;
                border-radius:5px;font-size:10px;opacity:0.92;
                max-height:50vh;width:220px;overflow:hidden;">
    <div style="display:flex;gap:4px;margin-bottom:4px;">
      <button onclick="document.getElementById('subList').style.display='block';
                       document.getElementById('lineList').style.display='none';
                       this.style.fontWeight='bold';this.nextElementSibling.style.fontWeight='normal';"
              style="font-size:10px;font-weight:bold;flex:1;padding:2px">変電所</button>
      <button onclick="document.getElementById('lineList').style.display='block';
                       document.getElementById('subList').style.display='none';
                       this.style.fontWeight='bold';this.previousElementSibling.style.fontWeight='normal';"
              style="font-size:10px;flex:1;padding:2px">送電線</button>
    </div>
    <input type="text" id="listSearch" placeholder="検索..."
           oninput="var q=this.value.toLowerCase();
                    document.querySelectorAll('.list-item').forEach(function(el){{
                      el.style.display=el.textContent.toLowerCase().includes(q)?'block':'none';
                    }})"
           style="width:100%;font-size:10px;padding:2px;margin-bottom:4px;box-sizing:border-box">
    <div id="subList" style="overflow-y:auto;max-height:calc(50vh - 80px);">
      {sub_items}
    </div>
    <div id="lineList" style="overflow-y:auto;max-height:calc(50vh - 80px);display:none;">
      {line_items}
    </div>
    </div>
    <style>.list-item:hover{{background:#f0f0f0}}</style>
    """
    m.get_root().html.add_child(folium.Element(list_panel_html))

    # Foliumのマップ変数名を取得してリストのonclickと連携
    map_var = m.get_name()
    js_fix = f"""
    <script>
    ...
    output_map = OUTPUT_DIR / "tochigi_mesh_multi_map.html"
